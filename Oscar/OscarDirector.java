...
//main class
public class OscarDirector extends JFrame {
  ...
  String s2 = "Shapefile/OscarAward.shp"; 
  String datapathname = "";
  ...
  public OscarDirector() {

    super("Oscar Best Foreign Film");
  ...
  class ContactUs extends JFrame implements ActionListener {
	public ContactUs() {
		...
		"USA<BR><BR>" +
		"Email1 :  ztimemakercl@gmail.com<BR>";
  ...
  public static void main(String[] args) throws IOException {
	  ...
	  BufferedImage myPicture = ImageIO.read(new File("E:\\Eclipse_workspace\\TestProject\\src\\Oscar\\welcomeImg.jpg"));
	  panel.setSize(900,500);
	  panel.setBackground(Color.white);
	  JLabel label = new JLabel();
	  label.setSize(900, 500);
	  Image scaledImage = myPicture.getScaledInstance(label.getWidth(),label.getHeight(),Image.SCALE_SMOOTH);
	  ImageIcon image = new ImageIcon(scaledImage);
	  label.setIcon(image);
	  panel.add(label);
	  
	  JPanel panelBottom = new JPanel();
	  JButton button = new JButton("Go Explore!");
	  ...
	  button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae){
				...
				OscarDirector qstart = new OscarDirector();
			    ...
			    qstart.setLocationRelativeTo(null);
			    env = map.getExtent();
			}
			});
	  ...
	  public void actionPerformed(ActionEvent ae) {
	    ...
	    if (source == cancel)
	      ...
	      try {
			...
			map.redraw();
			if (OscarDirector.stb.getSelectedLayers() != null)
			  promoteitem.setEnabled(true);
		  } catch(IOException e){}
	    }
	  }
...
class XYfeatureLayer extends BaseFeatureLayer {
  ...
  public XYfeatureLayer(BasePointsArray bpa,Map map,Vector s2) {
	...
	BaseFeatureClass bfc = getFeatureClass("Great Director",bpa);
	setFeatureClass(bfc);
	...
	rms.setImageString("E:\\Eclipse_workspace\\TestProject\\src\\Oscar\\oscar_icon.png");
	rms.setSizeX(40);
	rms.setSizeY(40);
	srd.setSymbol(rms);
	setRenderer(srd);
...
class AttrTab extends JDialog {
  JPanel panel1 = new JPanel();
  com.esri.mo2.map.dpy.Layer layer = OscarDirector.layer4;
...
class CreateShapeDialog extends JDialog {
  ...
  ActionListener lis = new ActionListener() {public void actionPerformed(ActionEvent ae) {
	...
	if (o == nameField) {
	  ...
	  path = ((ShapefileFolder)(OscarDirector.layer4.getLayerSource())).getPath();
	  //path = "ShapeFile";
...
class DistanceTool extends DragTool  {
  ...
  public static void resetDist() { // undo measure tool residue
	  OscarDirector.milesLabel.setText("DIST   0 mi   ");
	  OscarDirector.kmLabel.setText("   0 km    ");
	  OscarDirector.map.remove(OscarDirector.acetLayer);
	  OscarDirector.acetLayer = null;
	  OscarDirector.map.repaint();
  }
  public void mousePressed(MouseEvent me) {
	startx = me.getX(); starty = me.getY();
	initPoint = OscarDirector.map.transformPixelToWorld(me.getX(),me.getY());
  }
  public void mouseReleased(MouseEvent me) {
	  // now we create an acetatelayer instance and draw a line on it
	endx = me.getX(); endy = me.getY();
	endPoint = OscarDirector.map.transformPixelToWorld(me.getX(),me.getY());
    ...
    OscarDirector.milesLabel.setText("DIST: " + new Float((float)distance).toString() + " mi  ");
    OscarDirector.kmLabel.setText(new Float((float)(distance*1.6093)).toString() + " km");
    if (OscarDirector.acetLayer != null)
    	OscarDirector.map.remove(OscarDirector.acetLayer);
    OscarDirector.acetLayer = new AcetateLayer() {
    ...
    OscarDirector.map.add(OscarDirector.acetLayer);
    OscarDirector.map.redraw();
  }
  public void cancel() {};
}
...
class HotPick extends JDialog {
  String mycity = OscarDirector.mycity;
  String mypic = null;
  String mycitypic = null;
  JPanel jpanel = new JPanel();
  JPanel jpanel2 = new JPanel();
  
  JButton button = new JButton("Famous Movie");
 	
	String[][] OscarMovies ={
			{"Canada","http://www.imdb.com/title/tt0338135/?ref_=fn_al_tt_1","Canada.jpg"},
			{"Japan","http://www.imdb.com/title/tt0042876/?ref_=fn_al_tt_1","Japan.jpg"},
			{"Germany","http://www.imdb.com/title/tt0078875/?ref_=fn_al_tt_1","Germany.jpg"},
			{"Netherlands","http://www.imdb.com/title/tt0119448/?ref_=fn_al_tt_1","Netherlands.jpg"},
			{"Poland","http://www.imdb.com/title/tt2718492/?ref_=nm_knf_i1","Poland.jpg"},
			{"Czech Repub","http://www.imdb.com/title/tt0060802/?ref_=fn_al_tt_1","Czech_Repub.jpg"},
			{"France","http://www.imdb.com/title/tt0301414/?ref_=fn_al_tt_","France.jpg"},
			{"Hungary","http://www.imdb.com/title/tt3808342/?ref_=fn_al_tt_1","Hungary.jpg"},
			{"Switzerland","http://www.imdb.com/title/tt0100470/?ref_=fn_al_tt_1","Switzerland.jpg"},
			{"Italy","http://www.imdb.com/title/tt0056801/mediaviewer/rm3041196800","Italy.jpg"},
			{"Spain","http://www.imdb.com/title/tt0185125/?ref_=fn_al_tt_1","Spain.jpg"},
			{"China","http://www.imdb.com/title/tt0190332/?ref_=fn_al_tt_1","China.jpg"},
			{"Iran","http://www.imdb.com/title/tt1846445/?ref_=fn_al_tt_1","Iran.jpg"},
			{"Ivory Coast","http://www.imdb.com/title/tt0074972/?ref_=fn_al_tt_1","Ivory_Coast.jpg"},
			{"Argentina","http://www.imdb.com/title/tt1305806/?ref_=fn_al_tt_1","Argentina.jpg"},
			{"Australia","http://www.imdb.com/title/tt0813547/?ref_=fn_al_tt_1","Australia.jpg"},
			{"South Africa","http://www.imdb.com/title/tt0468565/?ref_=fn_al_tt_1","South_Africa.jpg"},
			{"Soviet Union","http://www.imdb.com/title/tt0063794/?ref_=fn_al_tt_4","Russia.jpg"},
			{"Algeria","http://www.imdb.com/title/tt0065234/?ref_=nm_knf_i3","Algeria.jpg"},
			{"Bosnia/Herz","http://www.imdb.com/title/tt0283509/?ref_=nm_knf_i1","Bosnia_Herz.jpg"},
			{"Denmark","http://www.imdb.com/title/tt0093713/?ref_=fn_al_tt_1","Denmark.jpg"},
			{"Sweden","http://www.imdb.com/title/tt0055499/","Sweden.jpg"}
	};
	
  HotPick() throws IOException {
    setTitle("The Oscar Best Foreign Movie in this country");
    setBounds(25,25,550,700);
    addWindowListener(new WindowAdapter() {
      public void windowClosing(WindowEvent e) {
    ...
    for (int i = 0;i<21;i++)  {
      if (OscarMovies[i][0].equals(mycity)) {
        mypic = OscarMovies[i][1];
        mycitypic = OscarMovies[i][2];
